...
class DetectionHead(nn.Module):
    def __init__(self, in_channels, num_classes, num_anchors=3):
        ...
        prior = 0.01
        nn.init.constant_(self.cls_branch[-1].bias,
                           -math.log((1 - prior) / prior))
        nn.init.constant_(self.obj_branch[-1].bias,
                           -math.log((1 - prior) / prior))
...
class GerberEncoder(nn.Module):
    ...
    def forward(self, gerber):
        return self.proj(self.encoder(gerber))  # [B, C, H/16, W/16]


# ── Lightweight Cross-Modal Fusion ────────────────────────────────────────────
class CrossModalFusion(nn.Module):
    """
    Memory-efficient cross-modal fusion using depthwise convolution.
    Avoids full spatial attention maps which OOM at 640px.

    Strategy:
      1. Resize Gerber features to match image feature spatial size
      2. Concatenate along channel dim
      3. Fuse with depthwise-separable conv
      4. Residual add + norm
    """
    def __init__(self, channels=256):
        super().__init__()
        # Fuse concatenated [image + gerber] features
        self.fuse = nn.Sequential(
            # Depthwise
            nn.Conv2d(channels * 2, channels * 2, 3,
                      padding=1, groups=channels * 2),
            # Pointwise
            nn.Conv2d(channels * 2, channels, 1),
            nn.BatchNorm2d(channels),
            nn.ReLU(inplace=True),
        )
        # Gating: learn how much to trust design prior
    ...
    def forward(self, image_feat, gerber_feat):
        ...
        # Resize gerber to match image spatial dims
        gerber_resized = F.interpolate(
            gerber_feat, size=(H, W),
            mode='bilinear', align_corners=False)

        # Concatenate
        combined = torch.cat([image_feat, gerber_resized], dim=1)  # [B, 2C, H, W]

        # Depthwise-separable fusion
        fused = self.fuse(combined)   # [B, C, H, W]

        # Gating — how much design info to inject
        gate  = self.gate(combined).view(B, C, 1, 1)
        out   = image_feat + gate * fused

        return self.norm(out)


# ── Main Model ────────────────────────────────────────────────────────────────
class GerberFormerV2(nn.Module):
    def __init__(self, num_classes=NUM_CLASSES, num_anchors=NUM_ANCHORS,
        ...
        self.neck           = FPNNeck(backbone_channels, fpn_channels)
        self.gerber_encoder = GerberEncoder(fpn_channels)
        self.fusion_layers  = nn.ModuleList([
            CrossModalFusion(fpn_channels)
        ...
        print(f'[GerberFormerV2] {n:,} parameters | {len(backbone_channels)} scales')

    def forward(self, images, gerber=None):
        feats = self.backbone(images)
        feats = self.neck(feats)

        if gerber is not None:
            gerber_feat = self.gerber_encoder(gerber)
            feats = [
                fusion(f, gerber_feat)
                for fusion, f in zip(self.fusion_layers, feats)
            ]

        outputs = []
        for f, head in zip(feats, self.heads):
            cls, reg, obj, unc = head(f)
            outputs.append({'cls': cls, 'reg': reg,
                            'obj': obj, 'unc': unc})
        return outputs
    ...
    for scale_idx, pred in enumerate(predictions):
        ...
        for b in range(B):
            ...
            for box in gt:
                ...
                reg_target[b, gj, gi, best_a]         = torch.stack(
                    [cx, cy, bw, bh])
...
class GerberFormerV2Loss(nn.Module):
